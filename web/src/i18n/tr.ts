...
chat: {
  ...
  config: {
    ...
    categories: {
      ...
      discord: "Discord",
      auxiliary: "Yardımcı",
      agent_frameworks: "Agent Altyapıları",
    },
    fieldLabels: {
      "model": "Model",
      ...
      "agent.max_turns": "Maksimum Dönüş Sayısı",
      "agent.gateway_timeout": "Ağ Geçidi Zaman Aşımı",
      "agent.restart_drain_timeout": "Yeniden Başlatma Boşaltma Süresi",
      "agent.api_max_retries": "API Maksimum Yeniden Deneme",
      "agent.service_tier": "Hizmet Kademesi",
      "agent.tool_use_enforcement": "Araç Kullanımını Zorunlu Kıl",
      "agent.gateway_timeout_warning": "Zaman Aşımı Uyarısı",
      "agent.clarify_timeout": "Açıklama Zaman Aşımı",
      "agent.gateway_notify_interval": "Bildirim Aralığı",
      "agent.gateway_auto_continue_freshness": "Otomatik Devam Tazeliği",
      "agent.image_input_mode": "Resim Giriş Modu",
      "agent.disabled_toolsets": "Devre Dışı Araç Setleri",
      "terminal.backend": "Terminal Arka Ucu",
      "terminal.modal_mode": "Modal Modu",
      "terminal.cwd": "Terminal Çalışma Dizini",
      "terminal.timeout": "Terminal Zaman Aşımı",
      "terminal.env_passthrough": "Ortam Değişkenleri Geçişi",
      "terminal.shell_init_files": "Kabuk Başlatma Dosyaları",
      "terminal.auto_source_bashrc": "Bashrc'yi Otomatik Kaynak Al",
      "terminal.docker_image": "Docker İmajı",
      "terminal.docker_forward_env": "Docker Ortam Aktarımı",
      "terminal.docker_env": "Docker Ortam Değişkenleri",
      "terminal.singularity_image": "Singularity İmajı",
      "terminal.modal_image": "Modal İmajı",
      "terminal.daytona_image": "Daytona İmajı",
      "terminal.vercel_runtime": "Vercel Çalışma Zamanı",
      "terminal.container_cpu": "Konteyner CPU",
      "terminal.container_memory": "Konteyner Bellek",
      "terminal.container_disk": "Konteyner Disk",
      "terminal.container_persistent": "Kalıcı Konteyner",
      "terminal.docker_volumes": "Docker Bağlama Noktaları",
      "terminal.docker_mount_cwd_to_workspace": "Cwd'yi Workspace'e Bağla",
      "terminal.docker_run_as_host_user": "Ana Kullanıcı Olarak Çalıştır",
      "tts.provider": "TTS Sağlayıcısı",
      "stt.provider": "STT Sağlayıcısı",
      "display.skin": "Arayüz Teması (CLI)",
      "dashboard.theme": "Panel Teması",
      "display.resume_display": "Geçmiş Gösterimi",
      "display.busy_input_mode": "Meşgul Giriş Modu",
      "display.accessibility_contrast": "Renk Kontrastı",
      "display.accessibility_text_scale": "Yazı Boyutu",
      "display.accessibility_font_style": "Yazı Tipi Ailesi",
      "memory.provider": "Bellek Sağlayıcısı",
      "approvals.mode": "Tehlikeli Komut Onay Modu",
      "context.engine": "Bağlam Motoru",
      "human_delay.mode": "İnsan Gecikmesi Modu",
      "logging.level": "Günlük Seviyesi",
      "agent_frameworks.active_framework": "Aktif Agent Altyapısı",
      "agent_frameworks.langchain_endpoint": "LangChain Uç Noktası",
      "agent_frameworks.langchain_script": "LangChain Betiği",
      "agent_frameworks.crewai_endpoint": "CrewAI Uç Noktası",
      "agent_frameworks.crewai_script": "CrewAI Betiği",
      "agent_frameworks.autogen_endpoint": "AutoGen Uç Noktası",
      "agent_frameworks.autogen_script": "AutoGen Betiği"
    },
    fieldDescriptions: {
      "model": "Varsayılan model (örn. anthropic/claude-sonnet-4.6)",
      "model_context_length": "Bağlam penceresi aşımı (0 = model meta verilerinden otomatik algıla)",
      "toolsets": "Kullanılacak varsayılan araç setleri",
      "fallback_providers": "Birincil sağlayıcı başarısız olduğunda kullanılacak yedek sağlayıcı listesi",
      "agent.max_turns": "Bir oturumda izin verilen maksimum agent dönüş sayısı",
      "agent.gateway_timeout": "Ağ geçidi agent yürütmesi için hareketsizlik zaman aşımı (saniye). 0 = sınırsız.",
      "agent.restart_drain_timeout": "Ağ geçidinin durması veya yeniden başlaması için zarif boşaltma süresi (saniye).",
      "agent.api_max_retries": "API hataları için maksimum uygulama düzeyinde yeniden deneme sayısı.",
      "agent.service_tier": "API hizmet kademesi (OpenAI/Anthropic). Boş bırakılırsa varsayılan.",
      "agent.tool_use_enforcement": "Modele açıklamalar yapmak yerine doğrudan araçları çağırmasını söyleyen zorlama.",
      "agent.gateway_timeout_warning": "Tam zaman aşımından önce kullanıcıya gönderilecek hareketsizlik uyarısı eşiği (saniye).",
      "agent.clarify_timeout": "Ağ geçidinin kullanıcıdan açıklama istemi yanıtı bekleyeceği maksimum süre (saniye).",
      "agent.gateway_notify_interval": "Periyodik 'hala çalışıyor' durum mesajı gönderim aralığı (saniye).",
      "agent.gateway_auto_continue_freshness": "Kesintiye uğrayan oturumun otomatik devam notu enjekte edilme penceresi (saniye).",
      "agent.image_input_mode": "Kullanıcı tarafından eklenen resimlerin ana modele her dönüşte nasıl sunulacağı.",
      "agent.disabled_toolsets": "Devre dışı bırakılan araç setlerinin listesi.",
      "terminal.backend": "Terminal yürütme arka ucu (yerel, docker, ssh, modal, daytona, vb.).",
      "terminal.modal_mode": "Modal sandbox modu (sandbox veya function).",
      "terminal.cwd": "Terminalin varsayılan çalışma dizini.",
      "terminal.timeout": "Terminal komutları için maksimum çalışma süresi (saniye).",
      "terminal.env_passthrough": "Korumalı terminale aktarılacak ortam değişkenleri.",
      "terminal.shell_init_files": "Ortam anlık görüntüsü alınırken kaynak gösterilecek ekstra kabuk başlatma dosyaları.",
      "terminal.auto_source_bashrc": "Bash oturumlarında bashrc ve profile dosyalarını otomatik kaynak al.",
      "terminal.docker_image": "Docker korumalı alanında kullanılacak varsayılan Docker imajı.",
      "terminal.docker_forward_env": "Ana makineden Docker konteynerine aktarılacak ortam değişkenleri.",
      "terminal.docker_env": "Docker konteynerinin içinde ayarlanacak açık ortam değişkenleri.",
      "terminal.singularity_image": "Singularity korumalı alanında kullanılacak imaj.",
      "terminal.modal_image": "Modal korumalı alanında kullanılacak imaj.",
      "terminal.daytona_image": "Daytona korumalı alanında kullanılacak imaj.",
      "terminal.vercel_runtime": "Vercel Sandbox çalışma zamanı ortamı.",
      "terminal.container_cpu": "Korumalı alan konteynerleri için CPU limiti.",
      "terminal.container_memory": "Korumalı alan konteynerleri için bellek limiti (MB).",
      "terminal.container_disk": "Korumalı alan konteynerleri için disk limiti (MB).",
      "terminal.container_persistent": "Dosya sisteminin oturumlar boyunca kalıcı olması.",
      "terminal.docker_volumes": "Ana makine dizinlerini Docker konteyneri ile paylaşan birimler.",
      "terminal.docker_mount_cwd_to_workspace": "Docker oturumlarında ana makine çalışma dizinini /workspace altına bağla.",
      "terminal.docker_run_as_host_user": "Docker konteynerini ana makine kullanıcısının uid:gid değerleriyle çalıştır.",
      "tts.provider": "Metinden konuşmaya (Text-to-speech) sağlayıcısı.",
      "stt.provider": "Konuşmadan metne (Speech-to-text) sağlayıcısı.",
      "display.skin": "CLI görsel teması.",
      "dashboard.theme": "Web paneli görsel teması.",
      "display.resume_display": "Sürdürülen oturumların geçmişi nasıl göstereceği.",
      "display.busy_input_mode": "Agent çalışırken girdi davranışı.",
      "display.accessibility_contrast": "Erişilebilirlik: Renk kontrast modu.",
      "display.accessibility_text_scale": "Erişilebilirlik: Yazı tipi boyutu ölçeği.",
      "display.accessibility_font_style": "Erişilebilirlik: Yazı tipi ailesi stili.",
      "memory.provider": "Bellek sağlayıcı eklentisi.",
      "approvals.mode": "Tehlikeli komut onay modu.",
      "context.engine": "Bağlam yönetimi motoru.",
      "human_delay.mode": "Simüle edilmiş yazma gecikmesi modu.",
      "logging.level": "agent.log için günlük seviyesi.",
      "agent_frameworks.active_framework": "Aktif Yapay Zeka Agent Altyapısı Arka Ucu.",
      "agent_frameworks.langchain_endpoint": "LangChain API Ağ Geçidi Uç Noktası.",
      "agent_frameworks.langchain_script": "LangChain Yerel Orkestrasyon Betik Yolu.",
      "agent_frameworks.crewai_endpoint": "CrewAI API Ağ Geçidi Uç Noktası.",
      "agent_frameworks.crewai_script": "CrewAI Yerel Orkestrasyon Betik Yolu.",
      "agent_frameworks.autogen_endpoint": "AutoGen API Ağ Geçidi Uç Noktası.",
      "agent_frameworks.autogen_script": "AutoGen Yerel Orkestrasyon Betik Yolu."
    }
  },
