...
async def csp_header_middleware(request: Request, call_next):
    """Inject CSP + WCAG headers on all HTML responses.

    Runs before host_header_middleware so every HTML response (SPA, login
    page, error pages) gets the same security + accessibility headers.
    Survives hermes update via hermes-post-update-patch.sh.
    """
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        # WCAG: skip-link ekle (ekran okuyucu kullanıcıları için ana içeriğe atla)
        if "<a href=\"#main-content\"" not in html:
            html = html.replace(
                "<body>",
                '<body><a href="#main-content" class="skip-link" style="position:absolute;top:-9999px;left:1rem;z-index:9999;padding:0.75rem 1rem;background:#fff;color:#000;border:2px solid #000;font-weight:bold;" onfocus="this.style.top=\'1rem\'" onblur="this.style.top=\'-9999px\'">Ana içeriğe geç</a>',
                1,
            )
            html = html.replace('<div id="root">', '<div id="root" role="main" aria-label="AccessiMind Dashboard">')
        # CSP header
        csp_value = (
            "default-src 'self'; "
            "script-src 'self' 'unsafe-inline' 'unsafe-eval'; "
